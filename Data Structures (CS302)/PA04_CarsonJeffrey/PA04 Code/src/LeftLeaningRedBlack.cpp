...
//
//
//  MAX MIN AND ARRAYSIZE TEMPLATES
//
//
//
template<typename DataType>
DataType Max(DataType i1, DataType i2){
	if(i1 > i2){
		return i1;
	}
	else{
...
template<typename DataType>
DataType Min(DataType i1, DataType i2){
	if(i1 < i2){
		return i1;
	}
	else{
...
//
void LeftLeaningRedBlack::SanityCheckRec(LLTB_t *pNode, bool isParentRed, uint32_t blackDepth, uint32_t &minBlack, uint32_t &maxBlack)
{
	/*if (pNode->IsRed) {
...
//
uint32_t LeftLeaningRedBlack::KeyCount(void)
{
...
//	KeyCountRec()
//
uint32_t LeftLeaningRedBlack::KeyCountRec(LLTB_t *pNode)
{
	uint32_t tally = 0;
...
{	/*
	...
	// depth.
	uint32_t ary[64];
	for (uint32_t i = 0; i < ArraySize(ary); ++i) {
		ary[i] = 0;
	}
...
//	LeafDepthRec()
//
uint32_t LeftLeaningRedBlack::LeafDepthRec(LLTB_t *pNode, uint32_t depth, uint32_t ary[], uint32_t depthLimit, uint32_t blackDepth, uint32_t &minBlack, uint32_t &maxBlack)
{/*
...
//   locateNode()
//
LLTB_t * LeftLeaningRedBlack::locateNode(VoidRef_t ref){
	LLTB_t * tempNode = m_pRoot;
	uint32_t key = ref.Key;
	while(tempNode != nullptr){
		if(tempNode->Ref.Key == key){
			return tempNode;
		}
		else if(tempNode->Ref.Key < key){
...
//   locateParent()
//
LLTB_t * LeftLeaningRedBlack::locateParent(VoidRef_t ref){
	LLTB_t * tempNode = m_pRoot;
	uint32_t key = ref.Key;
	while(tempNode != nullptr){
		if(key == tempNode->Ref.Key){
			return nullptr;
		}
		if(tempNode->pLeft){
			if(tempNode->pLeft->Ref.Key == key){
				return tempNode;
			}
		}
		if(tempNode->pRight){
			if(tempNode->pRight->Ref.Key == key){
				return tempNode;
			}
		}
		if(tempNode->Ref.Key > key){
